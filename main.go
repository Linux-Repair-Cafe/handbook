...
func getMarkdown(path string) ([]byte, error) {
...
func getMarkup(md []byte) (*string, error) {
	reqBody := GithubMarkdownBody{
		Text:    string(md),
		Mode:    "gfm",
		Context: "linux-repair-cafe/handbook",
	}
...
func main() {
	language := flag.String("language", "", "the file to process relative to the current working directory")
	flag.Parse()

	if *language == "" {
		log.Fatalf("language must be specified")
	}
	wd, err := os.Getwd()
	if err != nil {
		log.Fatal(err)
	}
	markdownPath := fmt.Sprintf("%s/markdown/%s.md", wd, *language)
	if _, err := os.Stat(markdownPath); errors.Is(err, os.ErrNotExist) {
		log.Fatal("No such file ", markdownPath)
	}
	htmlPath := fmt.Sprintf("%s/html/%s.html", wd, *language)
	pdfPath := fmt.Sprintf("%s/pdf/%s.pdf", wd, *language)

	log.Printf("Opening markdown file %s...", markdownPath)
	markdown, err := getMarkdown(markdownPath)
	if err != nil {
		log.Fatal(err)
	}
